...
from .tcn_bin import MultibranchTemporalConvNet
from .bnn import BNNCONV3D,RPReLU

def conv1x3x3(in_planes, out_planes, stride=1):
    """1x3x3 convolution with padding"""
    return BNNCONV3D(in_planes, out_planes,
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, se=False):
        ...
        self.act1 = RPReLU(planes)
        self.conv2 = conv1x3x3(planes, planes)
        self.bn2 = nn.BatchNorm3d(planes)
        self.downsample = downsample
        self.stride = stride
        self.se = se
        self.act2 = RPReLU(planes)
...
class SingleBranchNet(nn.Module):
    def __init__(self, args):
        super(SingleBranchNet, self).__init__()
        self.args = args
...
class EV_BSTCNet(nn.Module):
    def __init__(self, args):
        super(EV_BSTCNet, self).__init__()
        self.args = args
        self.mbranch = SingleBranchNet(args)
