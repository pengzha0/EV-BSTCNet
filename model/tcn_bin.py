...
from .bnn import BNNConv1d,RPReLU1D
    ...
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, relu_type, dwpw=False):
        ...
        if dwpw:
            self.conv = nn.Sequential(
                # -- dw
                BNNConv1d(n_inputs, n_inputs, kernel_size, stride=stride,
            ...
            self.conv = BNNConv1d(n_inputs, n_outputs, kernel_size,
                                  stride=stride, padding=padding, dilation=dilation)
            self.batchnorm = nn.BatchNorm1d(n_outputs)
            self.chomp = Chomp1d(padding, True)
            self.non_lin = RPReLU1D(n_outputs)
...
class MultibranchTemporalConvNet(nn.Module):
    def __init__(self, num_inputs, num_channels, tcn_options, dropout=0.2, relu_type='prelu', dwpw=False):
        ...
        for i in range(tcn_options['num_layers']):
            dilation_size = 2 ** i
